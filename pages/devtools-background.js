...
let jetDebugger = new JetDebugger();
let created = false;
let getElementJETDetail = function(){
    let rtn = Object.create(null);
    if($0){
        rtn.contextFor = jetDebugger.contextFor();
        rtn.dataFor = jetDebugger.dataFor();
        rtn.dataForToJs = jetDebugger.dataForToJs();
        let bindings = jetDebugger.getBindings();
        rtn['bindings.cca'] = bindings.cca;
        rtn['bindings.ko'] = bindings.ko;
        let bindingsToJs = jetDebugger.getBindingsToJs();
        rtn['bindingsToJs.cca'] = bindingsToJs.cca;
        rtn['bindingsToJs.ko'] = bindingsToJs.ko;
    }
    return rtn;
}
chrome.devtools.panels.elements.createSidebarPane('JET',
    function (sidebar){
        function updateJETSidebar(){
            sidebar.setExpression("(" + getElementJETDetail.toString() + ")()");
        }
        updateJETSidebar();
        chrome.devtools.panels.elements.onSelectionChanged.addListener(updateJETSidebar);
	sidebar.onShown.addListener(updateJETSidebar);
    }
);

chrome.devtools.panels.create(
        'JET', 'img/jet_128.png', '/pages/devtools.html')